...
def cmd_send(args) -> int:
    ...
    if args.json:
        ...
        print(f"na fila ({result['id']}): {result['subject']} → {', '.join(result['to'])}")
        print("nada sai até você aprovar — 'mailforai pending' mostra a fila")
    else:
...
def cmd_pending(args) -> int:
    ...
    if pedidos and any(p["status"] == "pending" for p in pedidos):
        print("aprovar: mailforai approve <id>    recusar: mailforai reject <id>")
...
def cmd_approve(args) -> int:
    ...
    if pedido["status"] == "sent":
        print(f"enviado: {pedido['subject']} → {', '.join(pedido['to'])}")
    else:
...
def cmd_reject(args) -> int:
    try:
        ...
        return _fail(str(exc))
    notify.refresh_waiting()
    _out(pedido, True) if args.json else print(f"recusado: {pedido['subject']}")
    return 0
...
def cmd_answer(args) -> int:
    try:
        ...
        return _fail(str(exc))
    notify.refresh_waiting()
    _out(pergunta, True) if args.json else print(f"respondido: {pergunta['question']}")
    return 0
...
def cmd_mode(args) -> int:
    ...
    if args.json:
        ...
        return 0
    print(f"conta '{name}': modo {atual}")
    print("  auto     a IA envia sozinha, respeitando allowlist e teto"
          if atual == "auto" else
          "  confirm  a IA cria um pedido e nada sai até você aprovar")
    return 0
