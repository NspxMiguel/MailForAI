"""Senha da caixa: chaveiro do sistema, nunca em arquivo de configuração.

macOS usa o Keychain (`security`), Linux usa libsecret (`secret-tool`).
Em qualquer sistema, a variável de ambiente MAILFORAI_SECRET_<CONTA> tem
prioridade — é o caminho para CI e contêiner, onde não há chaveiro.
"""
...
def env_var_name(account: str) -> str:
    ...
    return f"MAILFORAI_SECRET_{slug}"


def _backend() -> str:
    if os.environ.get("MAILFORAI_KEYRING") == "none":
        return "none"
...
def set_secret(account: str, secret: str) -> str:
    """Grava a senha e devolve o nome do backend usado."""
    backend = _backend()
    if backend == "macos":
    ...
    raise KeyringError(
        ...
        f"{env_var_name(account)} antes de enviar"
    )
...
def get_secret(account: str) -> str:
    ...
    if backend == "macos":
        ...
        if proc.returncode == 0 and proc.stdout:
            return proc.stdout.decode().rstrip("\n")
    raise KeyringError(
        f"senha da conta '{account}' não encontrada. Grave com "
        f"'mailforai secret {account}' ou exporte {env_var_name(account)}."
    )
